...
#define CACHE_BCOPY(cmd, blk, bofs, p, nbytes)  \
  if (cmd == Read)                                                      \
    {                                                                   \
      switch (nbytes) {                                                 \
      case 1:                                                           \
        *((byte_t *)p) = CACHE_BYTE(&blk->subblocks->data[0], bofs); break;        \
      case 2:                                                           \
        *((half_t *)p) = CACHE_HALF(&blk->subblocks->data[0], bofs); break;        \
      case 4:                                                           \
        *((word_t *)p) = CACHE_WORD(&blk->subblocks->data[0], bofs); break;        \
      default:                                                          \
        { /* >= 8, power of two, fits in block */                       \
          int words = nbytes >> 2;                                      \
          while (words-- > 0)                                           \
            {                                                           \
              *((word_t *)p) = CACHE_WORD(&blk->subblocks->data[0], bofs); \
              p += 4; bofs += 4;                                        \
            }\
        }\
      }\
    ...
    {                                                                   \
      switch (nbytes) {                                                 \
      case 1:                                                           \
        CACHE_BYTE(&blk->subblocks->data[0], bofs) = *((byte_t *)p); break;        \
      case 2:                                                           \
        CACHE_HALF(&blk->subblocks->data[0], bofs) = *((half_t *)p); break;        \
      case 4:                                                           \
        CACHE_WORD(&blk->subblocks->data[0], bofs) = *((word_t *)p); break;        \
      default:                                                          \
        { /* >= 8, power of two, fits in block */                       \
          int words = nbytes >> 2;                                      \
          while (words-- > 0)                                           \
            {                                                           \
              CACHE_WORD(&blk->subblocks->data[0], bofs) = *((word_t *)p);         \
              p += 4; bofs += 4;                                        \
            }\
        }\
    }\
...
update_way_list(struct cache_set_t *set,        /* set contained way chain */
                struct cache_super_t *blk,        /* block to insert */
...
/* create and initialize a sub block cache structure */
...
cache_create(char *name,		/* name of the cache */
		 ...
		 unsigned int super_blk_size /* no of subblocks in a super block*/
			)	/* latency in cycles for a hit */
{
  struct cache_t *cp;
  struct cache_blk_t *blk;
  struct cache_super_t *super;
  int i, j, bindex;
  ...
  if (super_blk_size<=0)
    fatal("super block size `%d' must be a power of two", super_blk_size);
  if (!blk_access_fn)
    fatal("must specify miss/replacement functions");
//  if (policy==LRU) policy = LAEE;
//  else policy = LRU;
  ...
  cp->super_blk_size = super_blk_size;
  ...
  cp->set_mask = nsets-1;
  cp->tag_shift = cp->set_shift + log_base2(nsets)+log_base2(super_blk_size);
...
//    cp->sets[i].blks = CACHE_BINDEX(cp, cp->data, bindex);
      ...
      for (j=0; j<assoc; j++)
	{
	  ...
	  /* create super block */
	  struct cache_super_t *super_blk;
	  super_blk = malloc(sizeof(struct cache_super_t));
	  //super_blk = (struct cache_super_t)* calloc(1,32);
	  super_blk->tag = 0;
	  super_blk->ready = 0;
	  
	  struct cache_blk_t *sub_block_pointer[cp->super_blk_size];


    	  cp->sets[i].blks = super_blk;


	  for(int k=0; k < cp->super_blk_size;k++)
	  {
	  	super_blk->status[k] = 0;
	  	...
	  	/*attach the subblock*/
		sub_block_pointer[k]=blk;
	  }

	  super_blk->subblocks = sub_block_pointer;

	  
	  super_blk->user_data = (usize != 0
			    ? (byte_t *)calloc(usize, sizeof(byte_t)) : NULL);


	  /* insert into head of way list, order is arbitrary at this point */
	  super_blk->way_next = cp->sets[i].way_head;
	  super_blk->way_prev = NULL;
	  if (cp->sets[i].way_head)
	    cp->sets[i].way_head->way_prev = super_blk;
	  cp->sets[i].way_head = super_blk;
	  if (!cp->sets[i].way_tail)
	    cp->sets[i].way_tail = super_blk;
            ...
            md_addr_t addr)             /* address of block to probe */
{
  ...
  struct cache_super_t *blk;
    ...
    {	
      for(int i=0;i<4;i++)
      	{	
		if (blk->tag == tag && (blk->status[i] & CACHE_BLK_VALID))
          		return TRUE;
      	}
    }
  ...
  for (i=0; i<cp->nsets; i++)
    { 
      for (blk=cp->sets[i].way_head; blk; blk=blk->way_next)
        { 
	 for(int i =0;i<4;i++){
          if (blk->status[i] & CACHE_BLK_VALID)
            { 
              cp->invalidations++;
              blk->status[i] &= ~CACHE_BLK_VALID;
              
              if (blk->status[i] & CACHE_BLK_DIRTY)
                { 
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case LAEE:
  {
	repl = cp->sets[set].way_head;
	  ...
	  int i = 0;
	  int j = 0;
//	  num_cache_super = sizeof(struct cache_set_t)/sizeof(struct cache_super_t)
      for (blk=cp->sets[set].way_head; blk; blk=blk->way_next)
	{
		  //printf("LAEE for loop");
		  sum = 0;
		  for (j=0; j<4; j++){
			  sum += blk->status[j];
		  }
		  ...
		  //printf("blk status:%d %d %d %d\n",blk->status[0],blk->status[1],blk->status[2],blk->status[3]);
		  //printf("sum:%d\n",sum);
		  //printf("leastaggr:%d\n",leastaggr);
		  if (sum<leastaggr ){
			  leastaggr = sum;
...
//		  blk = blk->way_next;
  ...
  if (repl->status[i] & CACHE_BLK_VALID)
    {
      cp->replacements++;
	...
	*repl_addr = CACHE_MK_BADDR(cp, repl->tag, set);
      ...
      if (repl->status[i] & CACHE_BLK_DIRTY)
	{
  ...
  repl->tag = tag;
  for(int i=0;i<4;i++)
  repl->status[i] = CACHE_BLK_VALID;	/* dirty bit set on update */
